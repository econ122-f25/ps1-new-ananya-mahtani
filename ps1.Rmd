...
Whether or not cylinders in descending order were included as an attribute to arrange by in addition to miles per gallon in ascending order did not make a difference to the order of the data. This indicates that cars with lower miles per gallon are more likely to have a higher number of cylinders anyways. 
...
The inner_join has less rows than the left_join, since the inner join only outputs observations matched in both datasets. The average grade calculation also reveals that for students with only one grade entry, the average column is a duplicate. We are therefore able to tell when a student has multiple grades, as the average differs from each individual grade given the limited entries. 
...
The semi_join tells us students with valid grade records, while the first anti_join outlines a part of which students are omitted from the previous set. The second anti_join conpletes this, as it outlines which students have grades, but not demographics listed, in addition to what the previous anti_join outlines, which is the students who have demoographics, but no grades. Together, they validate the valud data entries outputted in the semi_join.
