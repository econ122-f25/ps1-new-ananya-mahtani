...
Whether or not cylinders in descending order were included as an
attribute to arrange by in addition to miles per gallon in ascending
order did not make a difference to the order of the data. This indicates
that cars with lower miles per gallon are more likely to have a higher
number of cylinders anyways.
...
mtcars_processed%>% mutate(hp_per_wt= hp/wt) %>% 
...
# Your code here
mtcars_processed %>% 
  group_by(cyl, am) %>% summarize(avg_mpg=mean(mpg), med_hp=median(hp), num_cars=n()) %>% arrange(cyl, desc(avg_mpg))
```

    ## # A tibble: 6 × 5
    ## # Groups:   cyl [3]
    ##     cyl    am avg_mpg med_hp num_cars
    ##   <dbl> <dbl>   <dbl>  <dbl>    <int>
    ## 1     4     1    28.1   78.5        8
    ## 2     4     0    22.9   95          3
    ## 3     6     1    20.6  110          3
...
product_sales_long<- product_sales_wide %>% 
  pivot_longer(c('Year_2020', 'Year_2021', 'Year_2022'), names_to= 'Year', values_to='Sales') %>% 
...
## The long format is much more scalable because each observation has its own row, allowing you to easily calculate year-over-year growth using functions like lag() once the data is arranged by time. In contrast, the wide format stores values for different years in separate columns, so comparing across years requires manually referencing and subtracting specific columns. This makes the code harder to generalize and automate as the number of years increases. For time-series calculations, the long format is generally preferred because it keeps the structure consistent and simplifies both the logic and scalability of the analysis.
...
student_grades %>% inner_join(students_demographics, by="student_id")
```

    ## # A tibble: 5 × 7
    ##   student_id course_id semester   grade_score student_name major enrollment_year
    ##   <chr>      <chr>     <chr>            <dbl> <chr>        <chr>           <dbl>
    ## 1 S001       CS101     Fall 2020           92 Alice        CS               2020
    ## 2 S002       MA201     Spring 20…          85 Bob          Math             2021
    ## 3 S003       PH301     Fall 2021           88 Charlie      Phys…            2020
    ## 4 S001       CS102     Spring 20…          78 Alice        CS               2020
    ## 5 S004       CS205     Spring 20…          90 David        CS               2022

``` r
student_grades %>% left_join(students_demographics, by ="student_id")
```
    ...
    ## 3 S003       PH301     Fall 2021           88 Charlie      Phys…            2020
    ## 4 S001       CS102     Spring 20…          78 Alice        CS               2020
    ## 5 S006       BI101     Fall 2021           75 <NA>         <NA>               NA
    ## 6 S004       CS205     Spring 20…          90 David        CS               2022

``` r
# Your code for advanced mutate after left join
student_grades %>% left_join(students_demographics, by ="student_id")%>% group_by(student_id, student_name)%>% summarize(average_grade= mean(grade_score))
```
...
The inner_join has less rows than the left_join, since the inner join
only outputs observations matched in both datasets. The average grade
calculation also reveals that for students with only one grade entry,
the average column is a duplicate. We are therefore able to tell when a
student has multiple grades, as the average differs from each individual
grade given the limited entries.
...
The semi_join tells us students with valid grade records, while the
first anti_join outlines a part of which students are omitted from the
previous set. The second anti_join conpletes this, as it outlines which
students have grades, but not demographics listed, in addition to what
the previous anti_join outlines, which is the students who have
demoographics, but no grades. Together, they validate the valud data
entries outputted in the semi_join.
